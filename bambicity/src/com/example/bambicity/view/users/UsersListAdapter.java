...
public class UsersListAdapter extends BaseAdapter implements OnClickListener{
    ...
    public UsersListAdapter(Activity a, ArrayList <UserResponseModel> friendsListResponse) {
    ...
    private class OnItemClickListener  implements OnClickListener{           
       ...
       OnItemClickListener(int position){
        ...
        public void onClick(View arg0) {
        	...
        	String userId = UserInfoModel.getInstance().getUsersList().get(mPosition).getUserId();
			FragmentManager fragmentManager = activity.getSupportFragmentManager();
			fragmentManager
					.beginTransaction()
					.replace(R.id.container,
						 new UserAccountFragment(UserInfoModel.getInstance().getUsersList().get(mPosition))).commit();
